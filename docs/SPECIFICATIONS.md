...
- **運用方針**:
  ...
  3. 収集対象は当日・前日のうち、`setlist_status` が `NORMAL` ではなく、既存 `setlists` がないライブとする。
     - `collect_after` がある公演は、現在時刻が `collect_after` 以降になった時点から対象にする。
     - `collect_after` がない公演は、未実施・公演中の誤収集を避けるため翌日だけ対象にする。
     - これにより、時刻が分かる公演は終演見込み後から収集し、フェスなど時刻不明の公演は安全側に倒して翌日収集する。
  ...
  9. 信頼度 (`confidence`) は、楽曲DB一致率、曲数の妥当性、複数投稿一致、ノイズの少なさをもとに 0〜1 で算出する。UVERworld 楽曲マスタへの一致率が低い候補は他アーティストのセトリとして除外する。UVERworld公式Xの固定ユーザーIDから取得した投稿は `official_setlist` を有効にし、信頼度を0.20加点する。表示名やハンドル名だけでは公式判定しない。
...
- **主要データ**:
  - `lives.starts_at`: 開演日時。タイムゾーンを含む任意項目 (`TIMESTAMPTZ`)。
  - `lives.collect_after`: セトリ候補の収集開始日時。タイムゾーンを含む任意項目 (`TIMESTAMPTZ`)。
  - `lives.timezone`: 公演現地のIANAタイムゾーン。未設定時の既定値は `Asia/Tokyo`。
  - `raw_setlists`: 収集したセットリスト候補、解析済み楽曲、信頼度、重複情報、元投稿URLを保持。
  - `collector_logs`: 取得件数、候補数、作成数、エラー数など、収集実行ごとの運用ログを保持。
- **公演時刻の登録**:
  1. 公式スケジュールのタイトルに開場・開演時刻がある場合、最後の時刻または「開演」と明記された時刻を `starts_at` として取り込む。
  2. 通常公演の `collect_after` 初期値は `starts_at` の3時間後とする。実際の終演見込みが異なる場合は管理画面で修正する。
  3. 公演現地のタイムゾーンはIANA形式（`Asia/Tokyo`、`Asia/Seoul`、`Asia/Taipei`等）で保持し、夏時間を含む現地日付の判定に使用する。
  4. 時刻を取得できない場合は両項目を `NULL` のまま保持し、翌日収集へフォールバックする。
  5. 既存公演へ公式情報を紐付ける場合、手動設定済みの時刻は上書きせず、未設定項目だけ補完する。
  6. 管理画面の開演日時・収集開始日時は、公演に設定したタイムゾーンの現地時刻として入力・表示する。
- **確認・確定フロー**:
  1. 監視処理がXから候補を収集し、`raw_setlists`へ下書き保存する。
  2. 候補が作成された場合は、公演単位で管理者へ通知する。
  3. 管理者が内容・曲順・元投稿を確認する。
  4. 承認した候補だけを正式なセットリストとして確定する。自動確定は行わない。
