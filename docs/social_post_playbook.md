...
UVERworldのライブデータを活用したX投稿ネタの生成・確認手順書。

この機能は、Xから実施済み公演のセットリスト候補を集める「SNSセットリスト自動収集」とは別系統である。

- **セットリスト自動収集**: Xの投稿を検索し、管理者確認用のセトリ下書きを作る。投稿はしない。
- **X投稿コンテンツ**: サイトの機能や蓄積データを紹介する投稿文を作り、管理者が確認してXの投稿画面から公開する。
- どちらも管理者の確認を必須とし、X APIによる完全自動投稿やセトリの自動確定は行わない。

## 管理画面での投稿フロー

`On This Day` はバックエンドが1日1回、該当データがある日に限って下書きを自動作成する。同じ日の再実行は重複作成しない。自動承認・自動投稿は行わない。

1. 「X投稿コンテンツ」でカテゴリを選び、投稿下書きを生成する
2. 本文を編集し、「チェック済みにする」を押す
3. 「Xで投稿」を押し、本文が入力されたXの投稿画面で最終確認して投稿する
4. 投稿後のX URLを管理画面へ入力し、「投稿済みにする」を押す

投稿URLを登録すると、投稿ID・URL・投稿日時が保存される。X APIを使った完全自動投稿は行わない。
