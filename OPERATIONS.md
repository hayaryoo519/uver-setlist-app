...
> ## ⚠️ 開発・プッシュの鉄の掟
> 1. **プッシュ前のローカル確認（絶対遵守）**
>    - いかなる変更も、必ずローカル環境（`localhost:8000`）で動作確認してからプッシュすること。
>    - ローカルで動かないものは、サーバーでも動かない。
> 2. **.envの管理**
>    - `.env` ファイルはGit管理に含めない。
>    - 必要な環境変数の変更は、必ずサーバー側で直接設定すること。
...
| **`dev`** | **開発・検証用**。本番デプロイ前に「本番と同じビルド」を確認する。 | 検証環境 (Port 9001) |
| **`feature/*`** | **作業用**。機能追加やバグ修正ごとに作成する。 | ローカル開発環境 |

### 環境の動作条件について
不具合の早期発見のため、**検証環境（Staging）は本番環境（Production）と完全に同じ設定（`NODE_ENV=production`）**で動作します。ビルド時のエラーチェックやセキュリティ制限（Helmet）も本番同様に適用されます。
...
| ブランチ | トリガー | デプロイ先 |
| :--- | :--- | :--- |
| `dev` | プッシュ時 | 検証環境 (Port 9000) |

ワークフロー: `.github/workflows/deploy-staging.yml`
