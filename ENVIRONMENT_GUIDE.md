# 環境構成ガイド

> [!TIP]
> **[インフラ運用・管理ガイド (INFRASTRUCTURE_MANAGEMENT.md)](./INFRASTRUCTURE_MANAGEMENT.md)**
> ドメイン、Nginx、UFW、および環境差異に関するより詳細なルールはこちらを参照してください。

> [!CAUTION]
> ## ⚠️ 本番データベースは絶対に直接操作しないでください ⚠️
> 
> このプロジェクトには**3つの独立した環境**があります。各環境は完全に分離されており、**本番環境のデータベースを誤って操作すると、ユーザーに影響が出ます**。
> 
> ### 環境の使い分け：
> - **開発・テスト** → ローカル環境 または 検証環境を使用
> - **本番確認のみ** → 本番環境は読み取り専用で確認するだけ
> - **データベース操作** → 必ずローカルまたは検証環境で実施
> 
> 🚨 **本番データベースへの書き込み・削除・変更は厳禁です！** 🚨

このプロジェクトには3つの環境があります。それぞれの目的と使い分けを理解してください。

## 環境一覧

| 環境 | 場所 | URL | データベース | 用途 |
|:---|:---|:---|:---|:---|
| **ローカル開発** | Windows PC | http://localhost:8000 | Docker Supabase (ポート54332) | 日常の開発・テスト |
| **検証（Staging）** | サーバー | http://192.168.0.13:9000 | Docker PostgreSQL (ポート54323) | 本番反映前の最終確認 |
| **本番（Production）** | サーバー | http://192.168.0.13:8000 | PostgreSQL (ポート5432) | ユーザーがアクセスする環境 |

---

## 1. ローカル開発環境

### データベース: Docker Supabase

**なぜSupabaseを使うのか？**
- **完全なバックエンドスタック**: PostgreSQL + Auth + Realtime + Storage + Row Level Security
- **本番に近い環境**: Supabaseが提供する機能を全てローカルでテスト可能
- **独立性**: 本番データを壊す心配なく、自由に実験・開発できる
- **簡単セットアップ**: Dockerで一発起動、データベースマイグレーションも管理しやすい

### 起動方法

既にDockerコンテナが起動している場合（通常はこちら）:
```powershell
# サーバーを起動
cd C:\Users\oault\.gemini\antigravity\scratch\uver-setlist-app
$env:NODE_ENV="production"; node server/index.js
```

Dockerコンテナを再起動する場合:
```powershell
docker restart supabase_db_marumie
```

### 接続設定（`server/.env`）
```bash
DB_USER=postgres
DB_PASSWORD=postgres
DB_HOST=localhost
DB_PORT=54332
DB_NAME=uver_app_db
```

### 確認
```powershell
# APIテスト
Invoke-WebRequest -Uri http://localhost:8000/api/lives -UseBasicParsing
# 期待: 200 OK, 607件のデータ
```

---

## 2. 検証環境（Staging）

### データベース: Docker PostgreSQL

シンプルなPostgreSQLコンテナ。本番と同じコード・設定で動作確認する環境。

### 起動方法

サーバーにSSHで接続:
```bash
ssh haya-ryoo@192.168.0.13
cd ~/apps/uver-setlist-staging
docker compose up -d --build
```

### 接続設定（サーバー上の`server/.env`）
```bash
DB_USER=postgres
DB_PASSWORD=postgres
DB_HOST=localhost
DB_PORT=54332
DB_NAME=uver_app_db
```

### 確認
```bash
curl http://192.168.0.13:9000/api/lives
```

---

## 3. 本番環境（Production）

### データベース: PostgreSQL (Bare Metal)

サーバーに直接インストールされたPostgreSQL。

### 起動方法

通常はバックグラウンドで常時稼働中。再起動が必要な場合:
```bash
ssh haya-ryoo@192.168.0.13
cd ~/apps/uver-setlist-app/server
NODE_ENV=production nohup node index.js > server.log 2>&1 &
```

### 接続設定（サーバー上の`server/.env`）
```bash
DB_NAME=uver_setlist_prod
DB_USER=haya-ryoo
DB_PASSWORD='A1oaulth!'
DB_HOST=localhost
DB_PORT=5432
```

### 確認
```bash
curl http://192.168.0.13:8000/api/lives
```

---

## 開発ワークフロー

### 日常の開発
1. **ローカル環境**で開発
2. GitHubにpush

### 本番反映前のテスト
1. サーバーにSSH接続
2. **検証環境**で`git pull`して動作確認
3. 問題なければ**本番環境**にデプロイ

---

## よくある質問

### Q: なぜローカルだけSupabaseなのか？
A: Supabaseは開発に便利な機能（Admin UI、リアルタイム機能、認証など）を提供しているため。本番はシンプルなPostgreSQLで十分ですが、開発体験を向上させるためにSupabaseを使用しています。

### Q: 本番DBに直接接続したい場合は？
A: セキュリティ上推奨されませんが、必要な場合はSSHトンネル経由で接続できます：
```bash
ssh -L 5433:localhost:5432 haya-ryoo@192.168.0.13
```
その後、`server/.env`の本番設定を有効化してください。

### Q: データベースのバックアップは？
A: 本番データベースは毎日深夜3時に自動バックアップされ、過去7日分が保持されます。

---

## トラブルシューティング

### ローカルでデータが見えない
```powershell
# Dockerコンテナの状態確認
docker ps -a | findstr supabase

# データベース接続確認
node -e "require('dotenv').config({ path: './server/.env' }); const { Pool } = require('pg'); const pool = new Pool({ user: process.env.DB_USER, host: process.env.DB_HOST, database: process.env.DB_NAME, password: process.env.DB_PASSWORD, port: process.env.DB_PORT }); pool.query('SELECT COUNT(*) FROM lives', (err, res) => { if (err) console.error('Error:', err.message); else console.log('Lives count:', res.rows[0].count); pool.end(); });"
```

### 本番サーバーでエラー
```bash
# ログ確認
ssh haya-ryoo@192.168.0.13 "tail -50 ~/apps/uver-setlist-app/server/server.log"

# プロセス確認
ssh haya-ryoo@192.168.0.13 "ps aux | grep 'node.*server'"
```
